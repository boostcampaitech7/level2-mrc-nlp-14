...
from datasets import load_dataset, load_from_disk, Dataset, concatenate_datasets, DatasetDict, Features, Sequence, Value
import os
...
class UseSimilarData:
    ...
    def get_processed_data(self) -> Dataset:
        ...
        for split in ['train', 'validation']:
            columns_to_remove_my = [col for col in ['document_id', '__index_level_0__'] if col in original_dataset[split].column_names]
            if columns_to_remove_my:
                original_dataset[split] = original_dataset[split].remove_columns(columns_to_remove_my)
            original_dataset[split] = original_dataset[split].map(
                    add_question_mark,
                ).cast(features)
        ...
        if self.data_args.use_sim_data and self.data_args.sim_dataset_names:
            ...
            if similar_datasets:
                ...
                raise ValueError("`use_sim_data`가 True로 설정되었지만 유사한 데이터셋이 로드되지 않았습니다.")

            combined_dataset = concatenate_datasets([original_dataset['train'], concatenated_similar])
